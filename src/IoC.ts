...
import PathologyRequestHandler, {
  IPathologyRequestHandler,
} from './api/request-handlers/PathologyRequestHandler';
...
const Adapter = class implements IocAdapter {
  ...
  get<T>(someClass: ClassConstructor<T>, action?: Action): T {
    ...
    childContainer
      .bind(environment.ioc.childContainerBind)
...
class IoC {
  ...
  private RegisterRequestHandler(): void {
    ...
    this.container
      .bind<IPathologyRequestHandler>(TYPES.PathologyRequestHandler)
      .to(PathologyRequestHandler);
  }
