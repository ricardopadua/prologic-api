...
export default class PathologysController {
  public constructor(
    @inject(TYPES.PathologyRequestHandler) private readonly _handle: IPathologyRequestHandler,
  ) {}
  ...
  @Put('/:id')
  public updatePathology(
    @Param('id') id: number,
    @Body({ validate: true }) request: UpdatePathologyRequest,
  ) {
    return this._handle.SendCommand(request);
  }
